...
def load_catalog(db_path: str) -> dict:
    """Loads the local SteamSpy catalog generated by steamspy_catalog_fetcher.py."""
    path = Path(db_path)
    if not path.exists():
...
def extract_reviews_by_tags(
    ...
    Output columns:
        app_id, app_name, user_id, country_code, language,
        review_text, voted_up, tag, date_created
    """
    ...
    country_map  = fetch_country_codes(unique_users, api_key)
    df["country_code"] = df["user_id"].map(country_map).fillna("")
